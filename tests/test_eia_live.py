...
class EiaLiveSmokeTest(unittest.TestCase):
    ...
    def test_guangzhou_public_acceptance_api(self):
        ...
        detail = self.scraper._fetch_guangzhou_detail(record_id)
    ...
    def test_guangzhou_static_feeds(self):
        for feed in REGIONS['guangzhou']['feeds']:
            if feed['type'] != 'static':
                continue
            _, list_soup, status = self.scraper._fetch_html_with_status(feed['list_url'])
            ...
            _, detail_soup, detail_status = self.scraper._fetch_html_with_status(detail_url)
    ...
    def test_dongguan_public_form_api(self):
        ...
        for feed in region['feeds']:
            payload = self.scraper._request_dongguan_page(region, feed, today, today, 1)
            bounded = self.scraper._request_dongguan_page(
            ...
            if feed['announcement_type'] != '受理公告' and payload['rows']:
                ...
                if acceptance_number:
                    related = self.scraper._fetch_dongguan_acceptance_by_number(
            ...
            if payload['rows']:
                lead = self.scraper._dongguan_row_to_lead(region, feed, payload['rows'][0])
                _, detail_soup = self.scraper.fetch_html(lead['source_url'])
                self.assertIsNotNone(detail_soup)
                self.assertGreater(len(detail_soup.find_all('table')), 0)
                self.assertTrue(
                    self.scraper._validate_dongguan_detail(detail_soup, feed, payload['rows'][0])
                )
                detail_checked += 1
