...
class EiaScraperTest(unittest.TestCase):
    ...
    def test_guangzhou_acceptance_maps_core_fields_without_downloading_files(self):
        ...
        self.scraper._fetch_guangzhou_detail = Mock()

        rows = self.scraper._scrape_guangzhou_acceptance_page(1)
        ...
        self.scraper._fetch_guangzhou_detail.assert_not_called()
    ...
    def test_guangzhou_acceptance_rejects_short_or_duplicate_page(self):
        ...
        with self.assertLogs('scraper.eia', level='ERROR'):
            self.assertIsNone(self.scraper._scrape_guangzhou_acceptance_page(1))
    ...
    def test_guangzhou_acceptance_rejects_cross_page_drift_and_missing_core_fields(self):
        ...
        self.assertEqual(len(self.scraper._scrape_guangzhou_acceptance_page(1)), 100)
        with self.assertLogs('scraper.eia', level='ERROR'):
            self.assertIsNone(self.scraper._scrape_guangzhou_acceptance_page(2))

        missing = load_json('guangzhou_acceptance.json')
        missing['data']['list'][0]['PROJECT_NAME'] = ''
        self.scraper._guangzhou_total = None
        self.scraper._guangzhou_seen_ids.clear()
        self.scraper.fetch = Mock(return_value=FakeResponse(payload=missing))
        self.scraper._fetch_guangzhou_detail = Mock(return_value=None)
        with self.assertLogs('scraper.eia', level='ERROR'):
            self.assertIsNone(self.scraper._scrape_guangzhou_acceptance_page(1))
    ...
    def test_post_response_accepts_created_status(self):
        ...
        payload = self.scraper._post_json(
    ...
    def test_guangzhou_static_feed_uses_fixed_type_and_last_span_date(self):
        ...
        self.scraper._fetch_html_with_status = Mock(side_effect=fetch_html)
        feed = REGIONS['guangzhou']['feeds'][2]

        rows = self.scraper._scrape_static_page(
    ...
    def test_original_static_adapter_still_parses_list_and_detail(self):
        ...
        self.scraper._fetch_html_with_status = Mock(side_effect=[
            (list_html, BeautifulSoup(list_html, 'html.parser'), 200),
            (detail_html, BeautifulSoup(detail_html, 'html.parser'), 200),
        ])

        rows = self.scraper._scrape_static_page(REGIONS['jiangmen'], 1)
    ...
    def test_static_404_ends_paging_but_source_failure_marks_task_failed(self):
        self.scraper._fetch_html_with_status = Mock(return_value=(None, None, 404))
        self.assertEqual(self.scraper._scrape_static_page(REGIONS['jiangmen'], 8), [])
        with self.assertLogs('scraper.eia', level='ERROR'):
            self.assertIsNone(self.scraper._scrape_static_page(REGIONS['jiangmen'], 1))

        self.scraper._scrape_guangzhou_page = Mock(return_value=None)
    ...
    def test_source_failure_marks_base_task_failed(self):
        ...
        self.scraper._scrape_guangzhou_page = Mock(return_value=None)
        self.scraper._scrape_dongguan_page = Mock(return_value=[])
        ...
        self.scraper._scrape_dongguan_page.assert_called_once()
    ...
    def test_dongguan_mapping_preserves_post_attachment_as_metadata(self):
        ...
        lead = self.scraper._dongguan_row_to_lead(region, feed, row)
    ...
    def test_dongguan_window_reads_public_detail_and_keeps_snapshot(self):
        ...
        self.assertEqual(self.scraper._fetch_html_with_status.call_count, 1)
    ...
    def test_dongguan_window_supplements_approval_by_acceptance_number(self):
        ...
        self.scraper._fetch_dongguan_feed = Mock(side_effect=fetch_feed)
        self.scraper._fetch_dongguan_acceptance_by_number = Mock(return_value=acceptance)
        self.scraper._fetch_html_with_status = Mock(return_value=(None, None, 404))
        ...
        self.scraper._fetch_dongguan_acceptance_by_number.assert_called_once_with(
            region,
    ...
    def test_dongguan_exact_acceptance_lookup_has_no_date_filter(self):
        region = REGIONS['dongguan']
        acceptance = load_json('dongguan_acceptance.json')['rows'][0]
        self.scraper._post_form = Mock(return_value={'total': 1, 'rows': [acceptance]})

        row = self.scraper._fetch_dongguan_acceptance_by_number(
            region, str(acceptance['HBTB_XH']), acceptance['HBTB_XMMC']
        )

        self.assertEqual(row['ID'], acceptance['ID'])
        form = self.scraper._post_form.call_args.args[1]
    ...
    def test_dongguan_detail_errors_fail_but_404_keeps_list_fields(self):
        ...
        self.scraper._fetch_html_with_status = Mock(return_value=(None, None, 403))
        with self.assertLogs('scraper.eia', level='ERROR'):
            self.assertIsNone(self.scraper._scrape_dongguan_window(
                region, date(2026, 7, 14), date(2026, 7, 14)
            ))
        ...
        self.scraper._fetch_html_with_status = Mock(return_value=(
    ...
    def test_dongguan_splits_multi_day_result_before_page_four(self):
        ...
        self.scraper._request_dongguan_page = Mock(side_effect=request)
        rows = self.scraper._fetch_dongguan_feed(
    ...
    def test_dongguan_splits_single_day_by_acceptance_number(self):
        ...
        self.scraper._request_dongguan_page = Mock(side_effect=request)
        with patch('scraper.eia._DONGGUAN_NUMBER_MAX', 100):
            rows = self.scraper._fetch_dongguan_feed(
    ...
    def test_dongguan_rejects_page_four_and_captcha_response(self):
        ...
        with self.assertRaises(ValueError):
            self.scraper._request_dongguan_page(
                region, feed, date(2026, 7, 14), date(2026, 7, 14), 4
            )

        response = FakeResponse(text="<script>alert('请输入验证码');</script>")
        self.scraper._post_response = Mock(return_value=response)
        with self.assertLogs('scraper.eia', level='ERROR'):
            payload = self.scraper._post_form('https://example.invalid/item.do', {})
        self.assertIsNone(payload)

        self.scraper._post_response = Mock(return_value=FakeResponse(payload={
            'total': 1,
...
class ZhaoqingScraperTest(unittest.TestCase):
    ...
    def test_zhaoqing_api_list_maps_core_fields(self):
        ...
        # 用 patch 临时将 region feeds 缩减为只有受理公告，简化 mock
        region = REGIONS['zhaoqing']
        single_feed = [region['feeds'][0]]  # 只测受理公告

        detail_response = FakeResponse(text=detail_html)
        # 模拟全量模式（无日期过滤）——fixture 的两条 article 都会被处理
        self.scraper.fetch = Mock(side_effect=[
            FakeResponse(payload=api_data),  # 受理公告列表 API
            detail_response,                  # 第一条 article 详情
            detail_response,                  # 第二条 article 详情
        ])
        with patch.dict(REGIONS, {'zhaoqing': dict(region, feeds=single_feed)}):
            # 全量模式：不设日期过滤
            with patch.object(self.scraper, '_zhaoqing_start_date', return_value=None):
                results = self.scraper._scrape_zhaoqing_page(REGIONS['zhaoqing'], 1)
    ...
    def test_zhaoqing_row_to_lead_maps_kv_fields(self):
        ...
        lead = self.scraper._zhaoqing_row_to_lead(
    ...
    def test_zhaoqing_page_greater_than_one_returns_empty(self):
        ...
        region = REGIONS['zhaoqing']
        result = self.scraper._scrape_zhaoqing_page(region, 2)
    ...
    def test_zhaoqing_api_failure_returns_none(self):
        ...
        region = REGIONS['zhaoqing']
        result = self.scraper._scrape_zhaoqing_page(region, 1)
    ...
    def test_zhaoqing_detail_failure_preserves_list_fields(self):
        ...
        api_data = load_json('zhaoqing_acceptance.json')
        # 创建空 articles 的 payload 给第二、三个 feed
        empty_api = dict(api_data, articles=[])

        # 用 patch 临时将 region feeds 缩减为只有受理公告
        region = REGIONS['zhaoqing']
        ...
        self.scraper.fetch = Mock(side_effect=[
            FakeResponse(payload=api_data),  # 受理公告列表 API
            None,                            # 第一条 article 详情失败
            None,                            # 第二条 article 详情失败
        ])
        with patch.dict(REGIONS, {'zhaoqing': dict(region, feeds=single_feed)}):
            # 全量模式：不设日期过滤，两条都会被处理
            with patch.object(self.scraper, '_zhaoqing_start_date', return_value=None):
    ...
    def test_zhaoqing_start_date_returns_none_below_threshold(self):
        ...
        with patch('app.models.Lead') as mock_lead:
            ...
            start_date = scraper._zhaoqing_start_date('肇庆市')
    ...
    def test_save_keeps_restricted_files_in_raw_data_and_project_level_dedup(self):
        row = load_json('dongguan_acceptance.json')['rows'][0]
        region = REGIONS['dongguan']
        acceptance = self.scraper._dongguan_row_to_lead(region, region['feeds'][0], row)
    ...
    def test_database_write_failure_is_not_reported_as_duplicate(self):
        row = load_json('dongguan_acceptance.json')['rows'][0]
        lead = self.scraper._dongguan_row_to_lead(
